...
**IMPORTANT**: Read the template at `skills/claude/code-review/templates/code-review-template.md` BEFORE delegating. When delegating to the documentation agent:

1. Provide the **template path** so the agent reads it and follows its section order exactly
2. Provide a **prior review** from `docs/code_reviews/` as a style reference (e.g., the most recent one)
