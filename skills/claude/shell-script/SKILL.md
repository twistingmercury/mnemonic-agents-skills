...
If usage documentation is written to a standalone file, use a lowercase snake_case, versioned filename with `Version`, `Date`, and `Notes` metadata. Before editing it, inspect Git history and upstream or remote-tracking refs. Never edit a published version; preserve it and create the next version. If publication status is uncertain, treat committed documentation as published. Canonical living files that require fixed paths may be updated in place.
